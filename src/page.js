...
async function startStatus(name) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      // Nothing to do across an offline/live transition: the server
      // publishes one continuous timeline — the sequence never resets and the
      // seam carries #EXT-X-DISCONTINUITY — so the player follows the stream
      // straight through. Rebuilding here is what used to cause the stall.
    } catch {
